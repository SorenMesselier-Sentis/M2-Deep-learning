...
class PositionalEncoding(nn.Module):
    def __init__(self, d_model, max_len=512, dropout=0.1):
        ...
        # Create constant 'pe' matrix with values dependent on position and model dimension
        pe = torch.zeros(max_len, d_model)
        position = torch.arange(0, max_len).float().unsqueeze(1)  # shape: (max_len, 1)
        div_term = torch.exp(
            torch.arange(0, d_model, 2).float() * -(math.log(10000.0) / d_model))  # shape: (d_model//2,)
    ...
    def forward(self, x):
        ...
        seq_len = x.size(1)
        pe = self.pe[:, :seq_len, :]  # Slice positional encoding based on current sequence length
        x = x + pe
